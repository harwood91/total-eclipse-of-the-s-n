...
class WP_Press_This {
	...
	/**
	 * Get the source's img and save them locally, for posterity, unless we can't.
	 *
	 * @since 4.2.0
	 * @access public
	 *
	 ...
	 *
	 * Excluded URLs include share-this type buttons, loaders, spinners, spacers, WordPress interface img,
	 * tiny buttons or thumbs, mathtag.com or quantserve.com img, or the WordPress.com stats gif.
	 *
	 * @ignore
	 * @since 4.2.0
	 *
	...
	private function _limit_img( $src ) {
		...
		if ( preg_match( '!/ad[sx]?/!i', $src ) ) {
			...
			// Classic WordPress interface img
			return '';
	...
	/**
	 * Get a list of img with no duplicates.
	 *
	 * @since 4.2.0
	 * @access public
	 *
		...
		if ( ! empty( $data['_images'] ) ) {
			foreach ( $data['_images'] as $src ) {
				...
				if ( in_array( $prot_relative_src, $this->images ) ||
					...
					// Skip: already selected or some type of avatar and we've already gathered more than 15 img.
					continue;
				}
