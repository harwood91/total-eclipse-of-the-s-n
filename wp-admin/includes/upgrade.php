...
function upgrade_100() {
	...
	$sql = "UPDATE $wpdb->options
		SET option_value = REPLACE(option_value, 'wp-links/links-img/', 'wp-img/links/')
		WHERE option_name LIKE %s
		AND option_value LIKE %s";
	$wpdb->query( $wpdb->prepare( $sql, $wpdb->esc_like( 'links_rating_image' ) . '%', $wpdb->esc_like( 'wp-links/links-img/' ) . '%' ) );
...
function make_site_theme_from_default($theme_name, $template) {
	...
	$images_dir = @ opendir("$default_dir/img");
	if ($images_dir) {
		while(($image = readdir($images_dir)) !== false) {
			if (is_dir("$default_dir/img/$image"))
				continue;
			if (! @copy("$default_dir/img/$image", "$site_dir/img/$image"))
				return;
			chmod("$site_dir/img/$image", 0777);
		}
	}
	@closedir($images_dir);
}
