...
FeaturedImage = Library.extend({
	...
	initialize: function() {
		...
		// img that are not initially loaded still appear.
...
GalleryAdd = Library.extend({
	 ...
	 */
	initialize: function() {
		// If a library wasn't supplied, create a library of img.
		if ( ! this.get('library') ) {
			this.set( 'library', wp.media.query({ type: 'image' }) );
		}
 ...
 *
 * Allows for selecting multiple img in the Insert Media workflow, and then
