 ...
 * resizing services for img. The hook must return an array with the same
 ...
 * 1. If false (default), img will be scaled, not cropped.
 * 2. If an array in the form of array( x_crop_position, y_crop_position ):
 *    - x_crop_position accepts 'left' 'center', or 'right'.
 *    - y_crop_position accepts 'top', 'center', or 'bottom'.
 *    Images will be cropped to the specified dimensions within the defined crop area.
 * 3. If true, img will be cropped to the specified dimensions using center positions.
 *
 * @since 2.9.0
 *
 ...
 * @param bool|array $crop   Optional. Whether to crop img to specified width and height or resize.
...
function wp_constrain_dimensions( $current_width, $current_height, $max_width = 0, $max_height = 0 ) {
	...
	/**
	 * Filters dimensions to constrain down-sampled img to.
	 *
	 * @since 4.1.0
	 *
 ...
 * 1. If false (default), img will not be cropped.
 * 2. If an array in the form of array( x_crop_position, y_crop_position ):
 *    - x_crop_position accepts 'left' 'center', or 'right'.
 *    - y_crop_position accepts 'top', 'center', or 'bottom'.
 *    Images will be cropped to the specified dimensions within the defined crop area.
...
/**
 * Helper function to test if aspect ratios for two img match.
 *
 * @since 4.6.0
 *
 ...
 *
 * A mime icon for files, thumbnail or intermediate size for img.
 *
...
function wp_get_attachment_image_src( $attachment_id, $size = 'thumbnail', $icon = false ) {
	...
	if ( ! $image ) {
		...
		if ( $icon && $src = wp_mime_type_icon( $attachment_id ) ) {
			...
			$icon_dir = apply_filters( 'icon_dir', ABSPATH . WPINC . '/img/media' );
...
function wp_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	 ...
	 * out img that are leftovers from previous versions.
	 */
	...
	/*
	 * Loop through available img. Only use img that are resized
	...
	foreach ( $image_sizes as $image ) {
		...
		// Filter out img that are from previous edits.
		if ( $image_edited && ! strpos( $image['file'], $image_edit_hash[0] ) ) {
			continue;
		}

		/*
		 * Filters out img that are wider than '$max_srcset_image_width' unless
 ...
 * @return string Converted content with 'srcset' and 'sizes' attributes added to img.
 */
function wp_make_content_images_responsive( $content ) {
	...
	if ( count( $attachment_ids ) > 1 ) {
		 ...
		 * warms the object cache with the meta information for all img.
		 */
 ...
 *
 *     @type string       $order      Order of the img in the gallery. Default 'ASC'. Accepts 'ASC', 'DESC'.
 *     @type string       $orderby    The field to use when ordering the img. Default 'menu_order ID'.
 ...
 *     @type int          $columns    Number of columns of img to display. Default 3.
 *     @type string|array $size       Size of the img to display. Accepts any valid image size, or an array of width
 *                                    and height values in pixels (in that order). Default 'thumbnail'.
...
function wp_playlist_shortcode( $attr ) {
	...
	$data = array(
		...
		'img' => wp_validate_boolean( $atts['img'] ),
